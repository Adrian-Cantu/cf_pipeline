...
foreach(@order) {
	...
	if (scalar(@f)==2) {
		...
		print MAIN "P01_prinseq_output/$f[0]_good_out_R1%fastq.gz P01_prinseq_output/$f[0]_good_out_R2%fastq.gz : P00_rawreads/$f[1] P00_rawreads/$f[2]\n";
#		print MAIN "\tscripts/quality\$*pl -f P00_rawreads/$f[1] -r P00_rawreads/$f[2] -t 8 -o $f[0]\n\n";
        print MAIN "\tpprinseqc -fastq P00_rawreads/$f[1] -fastq2 P00_rawreads/$f[2] -lc_entropy=0.5 -trim_qual_right=15 -trim_qual_left=15 -trim_qual_type mean -trim_qual_rule lt -trim_qual_window 2 -min_len 60 -min_qual_mean 15  -rm_header  -out_name P01_prinseq_output/$f[0] -threads 40 -out_gz  \n\n";
		print MAIN "P015_hisat_output/$f[0]_hg.sam : P01_prinseq_output/$f[0]_good_out_R1.fastq.gz P01_prinseq_output/$f[0]_good_out_R2.fastq.gz\n";
		print MAIN "\t\${hisat2} -x \${db_folder}/grch38/genome -1 P01_prinseq_output/$f[0]_good_out_R1.fastq.gz -2 P01_prinseq_output/$f[0]_good_out_R2.fastq.gz -S P015_hisat_output/$f[0]_hg.sam -q  --new-summary --time --summary-file  P015_hisat_output/$f[0]_log\n\n";
